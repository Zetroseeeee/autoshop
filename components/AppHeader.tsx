  ...
  return (
    <header className="mb-4">
      ...
      <div className="mt-3 flex items-center gap-1">
        <Link href="/" className="view-chip" data-active={!onRun} prefetch={false}>
          Basket
        </Link>
        <Link href="/run" className="view-chip" data-active={onRun} prefetch={false}>
          Checkout
        </Link>
        <Link href="/settings" aria-label="Settings" className="ml-auto flex h-9 w-9 items-center justify-center rounded-full text-grey hover:text-ink" prefetch={false}>
          <GearIcon />
        </Link>
      </div>
    </header>
