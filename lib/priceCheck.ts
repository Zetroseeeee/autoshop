...
/**
 * Daily price check (stretch): re-run Tier 1 on `want` items. On a drop, remember
...
async function checkOne(item: Item, summary: PriceCheckSummary): Promise<void> {
  ...
  if (found == null || !sameCurrency) {
    summary.noPrice++;
    await db.update(items).set({ lastCheckedAt: now }).where(eq(items.id, item.id));
    return;
  }
  ...
  if (found < item.priceMinor) {
    ...
    await db
      ...
      .where(and(eq(items.id, item.id), eq(items.priceMinor, item.priceMinor)));
    return;
  }
  if (found > item.priceMinor) {
    summary.rises++;
    await db.update(items).set({ priceMinor: found, previousPriceMinor: null, lastCheckedAt: now }).where(eq(items.id, item.id));
    return;
  }
  summary.unchanged++;
  await db.update(items).set({ lastCheckedAt: now }).where(eq(items.id, item.id));
